...
def writing_task(agent: Agent, style_guide: StyleGuide, brief: BriefSpec) -> Task:
    ...
    material = brief.material.strip()

    # 재료가 있으면 프롬프트에 통째로 싣고, 내용의 최우선 출처로 못 박는다.
    # 주제 한 줄만 받으면 모델이 일반 지식으로 일반론을 쓰게 되는데, 그 글은
    # 절끼리 유기성이 없다. 필자의 실제 경험이 글의 척추가 되게 한다.
    if material:
        material_block = f"""
[재료 — 필자가 직접 겪은 일과 메모. 이 글의 알맹이다]
{material}
"""
        material_priority = (
            "0. 본문의 내용은 [재료]를 최우선 출처로 삼는다. 재료와 일반 지식이"
            " 다르면 재료(필자의 경험)를 따른다.\n"
        )
        material_thought = (
            "  재료의 항목 하나하나를 어느 절에 배치할지 먼저 정한다. 어느 절에도"
            " 들어가지 못한 재료가 남으면 그 재료를 위한 절을 만든다.\n"
        )
        material_rules = (
            "- 재료 속 구체 사실(수치, 상황, 에러 메시지, 명령)을 본문에 실제로"
            " 쓴다. 뭉뚱그려 요약하지 않는다.\n"
            "- 재료를 그대로 복사해 붙이지 않는다. 문체 가이드와 말투에 맞게"
            " 다시 쓴다.\n"
        )
    else:
        material_block = ""
...
[우선순위 — 지시가 서로 어긋날 때]
{material_priority}1. 글의 구조는 문서 유형 템플릿을 따른다. 문체 가이드의 '전개 구조'보다 우선한다.
...
[반드시 지킬 것]
{material_rules}- 출력은 마크다운 본문만. 설명이나 머리말을 앞에 붙이지 않는다.
- 최상단에 `# 제목` 을 한 줄 넣는다.
...
def expand_task(agent: Agent, draft: str, brief: BriefSpec, current: int) -> Task:
    ...
    material_rule = (
        "- 재료 중 아직 본문에 반영되지 않은 것이 있으면 새 절에서 먼저 쓴다.\n"
        if material
        else ""
    )
    return Task(
...
{material_rule}- 주제 "{brief.topic}" 를 실제로 풍성하게 하는 절만 더한다. 배경, 작동 원리,
