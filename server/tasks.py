...
Action: 템플릿을 바탕으로 소제목 뼈대를 먼저 잡는다. 대괄호 자리는 주제에
  맞는 실제 제목으로 바꾸고, 반복 가능한 섹션(단계, 해결 방법 등)은 내용에
  필요한 만큼 늘린다.

Observation: 뼈대의 소제목(## 과 ###) 개수를 센다. 5개 미만이면 단계를 더
  쪼개거나 절을 더해 5개 이상으로 만든다.
...
- 문체 가이드의 donts 를 어기지 않는다."""
        ...
        expected_output=(
            "마크다운 블로그 초안 전문. 소제목 5개 이상. 코드펜스로 전체를 "
            "감싸지 말고 마크다운 본문 자체를 출력할 것."
        ),
        agent=agent,
    )
